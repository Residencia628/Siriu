...
export default function MarcasList() {
  ...
  const handleDelete = async (marcaId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${API}/marcas/${marcaId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Marca eliminada");
      fetchMarcas(); // Refresh the list
    } catch (error) {
      toast.error(error.response?.data?.detail || "Error al eliminar marca");
    }
  };

  if (loading) {
    return (
      <div className="flex items-center justify-center h-96" data-testid="marcas-loading">
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="marcas-list-container">
      {/* Header */}
      <div>
        <Button
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400">Gestiona las marcas disponibles para equipos</p>
          </div>
          {(user?.role === "admin" || user?.role === "superadmin") && (
            <Button
              onClick={() => navigate("/marcas/new")}
              data-testid="add-marca-button"
      ...
      {marcas.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-gray-500 mb-4">No hay marcas registradas</div>
          {(user?.role === "admin" || user?.role === "superadmin") && (
            <Button
              onClick={() => navigate("/marcas/new")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {marcas.map((marca) => (
            <div
              ...
              data-testid={`marca-card-${marca.id}`}
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {(user?.role === "admin" || user?.role === "superadmin") && (
                  <div className="flex space-x-2">
                    <Button
                      onClick={() => navigate(`/marcas/edit/${marca.id}`)}
                      size="sm"
